...
const Home: React.FC = () => {
  ...
  const [weatherType, setWeatherType] = useState<WeatherSelection>(
    weatherOptions[1]
  )
    ...
    const getData = async (): Promise<void> => {
      try {
        const { data } = await axios.get<Destination[]>(
          `/api/destinations/filter/?month=${month}&min_temp=${minTemp}&max_temp=${maxTemp}&weather_type=${weatherType.value}`
        )
        ...
        console.log(
          `Filtered destinations by ${month} and ${minTemp}-${maxTemp}`,
          data
        )
      } catch (err) {
        console.log(err)
      ...
      <header>
        ...
        <div className="above-gradient homepage">
          ...
          <h2 className="homepage-h2">
            Helping you find the best place to travel at{' '}
        ...
        <div id="control-container">
          ...
          <div id="dropdown-container">
            ...
            <button
              className="go-btn"
              onClick={() =>
                retrieveDestinations(month, minTemp, maxTemp, weatherType)
              }>
              Go!
            </button>
          </div>
