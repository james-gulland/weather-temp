...
const TemperatureControls: React.FC<Props> = ({
  minTemp,
  maxTemp,
  setMinTemp,
  setMaxTemp,
  ...
  return (
    <>
      ...
      <div id="min-temp-container">
        <div id="min-temp-number" className="tempNumber">
          {tempUnit === 'C' ? minTemp : celsiusToFahrenheit(minTemp)}
        </div>
        <div id="min-temp-controls">
          <button
            onClick={() => handleTempDecrement('min')}
            disabled={minTemp <= rangeMinTemp}>
            -
          </button>
          <button
            onClick={() => handleTempIncrement('min')}
            disabled={minTemp >= maxTemp || minTemp >= rangeMaxTemp}>
            +
          </button>
        </div>
      </div>
      ...
      <div id="max-temp-container">
        <div id="max-temp-number" className="tempNumber">
          {tempUnit === 'C' ? maxTemp : celsiusToFahrenheit(maxTemp)}
        </div>
        <div id="max-temp-controls">
          <button
            onClick={() => handleTempDecrement('max')}
            disabled={maxTemp <= minTemp}>
            -
          </button>
          <button
            onClick={() => handleTempIncrement('max')}
            disabled={maxTemp >= rangeMaxTemp}>
            +
          </button>
        </div>
      </div>
